...
var generateConstructor = function(inPrototype, inTemplate) {
	...
	// 4.b.1.1. Runs the custom element instantiation algorithm with PROTOTYPE and TEMPLATE as arguments.
...
var generatePrototype = function(inExtends, inProperties) {
	// 6.a.1 If EXTENDS is an invalid HTML tag name, throw an InvalidCharacterError exception.
	if (!inExtends) {
		throw "6.1. InvalidCharacterError: extends must be a valid HTML tag name";
	}
	// 6.a.2. If EXTENDS is a custom element name name, let BASE be the element prototype of the custom DOM element with the custom element name of EXTENDS
	/* TODO */
	
	// 6.a.3. Otherwise
	// 6.a.3.1 If BASE is defined in HTML specification or other applicable specifications, let BASE be the interface prototype object for the element type corresponding to the HTML tag name of EXTENDS
	...
	// 6.a.3.5 If PROPERTIES is present and not undefined, define properties on PROTOTYPE using PROPERTIES
	// TODO: determine if this is acceptable implementation of 'define properties'
...
var upgradeElements = function(inTree, inDefinition) {
	...
	for (var i=0; element=elements[i]; i++) {
		// 6.b.2.3. Let UPGRADE be the result of running custom element instantiation algorithm with PROTOTYPE and TEMPLATE as arguments
		...
		// 6.b.3 On UPGRADE, fire an event named elementupgrade with its bubbles attribute set to true.
		/* TODO */
	}
};

// SECTION 5 deals with UA parsing HTML: TODO: do we need to polyfill something here?
...
var validateArguments = function(inName, inOptions) {
	...
	// FIXME: wrong test
	if (!inName) {
		// 7.1.1. If NAME is an invalid custom element name, throw an InvalidCharacterError exception.
	...
	if (p && !(p instanceof HTMLElement)) {
		// 7.1.3. if PROTOTYPE does not inherit from the HTMLElement interface, throw a TypeMismatchError exception.
...
var register = function(inName, inOptions) {
	...
	//
	// TODO: resolve discrepancy between input and the formal arguments
	//
	validateArguments(inName, inOptions);
	// 7.1.2 If PROTOTYPE is missing, let PROTOTYPE be the interface prototype object for the HTMLSpanElement interface
	...
	// 7.1.6: For DOCUMENT tree and every shadow DOM subtree enclosed by DOCUMENT tree:
	/* TODO: and every shadow DOM subtree */
	// 7.1.6.1. Let TREE be this tree
	// 7/1.6.2. Run element upgrade algorithm with TREE and DEFINITION as arguments
	upgradeElements(document, definition);
	// 7.1.7. Run custom element constructor generation algorithm with PROTOTYPE and TEMPLATE as arguments
